...
struct ImaginationWorldView: View {
    ...
    @StateObject private var viewModel = WorldComicsViewModel()
    ...
    var body: some View {
            ...
            ZStack {
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 20) {
                        ...
                        // Stats section
                        if let stats = viewModel.worldStats {
                            HStack(spacing: 20) {
        ...
        .toolbar {
            CustomTopBarContent(title: "Fantasy World", showBackButton: true, leadingAction: {
        ...
        .onAppear {
            Task {
                await viewModel.loadWorldData(worldType: .imaginationWorld)
            }
        }
    }
}
    ...
    var body: some View {
        Button(action: {
            ...
            VStack(alignment: .leading, spacing: 8) {
                // Comic image
                if let imageData = Data(base64Encoded: comic.imageBase64) {
...
// MARK: - World Comics ViewModel
class WorldComicsViewModel: ObservableObject {
    @Published var comics: [ComicGenerationResponse] = []
    @Published var worldStats: WorldStatsResponse?
    @Published var isLoading = false
    @Published var errorMessage: String?
    @Published var showFavoritesOnly = false {
        didSet {
            if showFavoritesOnly != oldValue {
                Task { await loadComics() }
            }
        }
    }
    
    private let apiClient = APIClient.shared
    private var currentWorldType: WorldType = .imaginationWorld
    
    @MainActor
    func loadWorldData(worldType: WorldType) async {
        currentWorldType = worldType
        
        isLoading = true
        errorMessage = nil
        
        // Load both stats and comics in parallel
        async let statsTask = loadStats()
        async let comicsTask = loadComics()
    ...
    private func loadStats() async {
        do {
            worldStats = try await apiClient.getWorldStats(worldType: currentWorldType)
        } catch {
            print("❌ Failed to load world stats: \(error)")
        }
    }
    ...
    private func loadComics() async {
        do {
            comics = try await apiClient.getWorldComics(
            ...
            errorMessage = "Please log in to view your comics"
        } catch {
            errorMessage = "Failed to load comics: \(error.localizedDescription)"
        }
    }
}
